...
name_ip = input(
    'Введите адресную строку или просто нажмите Enter.\nПример http://04991447.ngrok.io \n')
f_f_f = True
if name_ip:
    ip = name_ip
FPS = 60
...
class Button:
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    p = event.pos
                    if 10 <= p[0] <= font.size('назад')[0] + 10 and \
                            30 <= p[1] <= font.size('назад')[1] + 30:
                        if i:
                            i -= 1
                        else:
                            self.flag = True
                            ...
                            return
                    elif 920 <= p[0] <= font.size('далее')[0] + 920 and 30 <= p[1] <= \
                            font.size('далее')[1] + 30:
                        if i + 1 < len(lst):
                            i += 1
    ...
    def update(self, pos, flag=False):
        ...
        if 20 <= x <= 30 + self.play[0] + 10 and 300 <= y <= 300 + self.play[1] and self.flag:
            ...
            if flag:
                return True
        elif 20 <= x <= 30 + self.new_map[0] + 10 and \
                340 <= y <= 340 + self.new_map[1] and self.flag:
            screen.blit(font.render(self.word[1], 1, (0, 0, 0)), (30, 340))
            if flag:
                self.map()
        elif 20 <= x <= 30 + self.records[0] + 10 and \
                380 <= y <= 380 + self.records[1] and self.flag:
    ...
    def score(self):
        ...
        result = list(map(lambda x: x[1] + ' = ' + str(x[0]),
                          sorted(result, reverse=True, key=lambda x: x[0])[:10]))
        self.render(result)
        font = pygame.font.Font('data/freesansbold.ttf', 20)
        screen.blit(font.render('назад', 1, (255, 255, 255)), (10, 30))
        pygame.display.flip()
...
class Player:
    ...
    def check_change(self):
        if self.board_pos != self.get_board_pos():
            self.board_pos = self.get_board_pos()
            self.x = self.get_board_pos(5, 5)[0] * self.board.cell_size + self.board.left
            self.y = self.get_board_pos(5, 5)[1] * self.board.cell_size + self.board.top
    ...
    def draw(self):
        pygame.draw.rect(self.screen, (200, 200, 10),
                         (self.x, self.y, self.board.cell_size, self.board.cell_size))
    ...
    def draw(self, *args):
        ...
        if self.color == "black" and self.collectable and self.collected:
            ...
            pygame.draw.circle(screen, (255, 255, 255),
                               (args[0] + args[2] // 2, args[1] + args[2] // 2), args[2] // 6)
...
class Merzopakostniy(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.step == 240:
            ...
            self.chek_lst.append(random.randint(300, 390))
            self.step = 0
        if not self.y and not self.x:
            while True:
                self.y = random.randint(1, (len(self.board.wall_map) - 3))
                self.x = random.randint(1, (len(self.board.wall_map[0]) - 3))
...
class Finish(pygame.sprite.Sprite):
    def __init__(self, group, record):
        ...
        time_delta = clock.tick(40) / 1000
        inname = act = False
        manager = pygame_gui.UIManager(size)
        name_hero = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect((200, 455), (350, 485)),
            manager=manager)

        while True:
            # print(self.name_hero.text)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.name_input(name_hero.text)
                    terminate()
                if not act:
                    continue
    ...
    def name_input(self, hero=''):
        ...
        result = cur.execute('''SELECT * from records where map="{}" AND name="{}"'''.
                             format(a, hero)).fetchall()
        if len(result):
            if result[0][0] < int(self.record):
                cur.execute('''UPDATE records SET point={} WHERE name="{}" AND map="{}"'''.
                            format(int(self.record), hero, a))
        else:
            cur.execute('''INSERT INTO records 
            VALUES ({},"{}","{}")'''.format(int(self.record), hero, a))
        con.commit()
        try:
            maps = a[:a.find('.')]
            glaf = True
            if maps in [i['name_map'] for i in
                        get(ip + '/api/maps').json()['maps']]:
                users = get(ip + '/api/user').json()['users']
                for user in users:
                    if hero == user['name']:
                        for rec in get(ip + '/api/records/' +
                                       maps).json()['records']:
                            if user['id'] == rec['user_id']:
                                if rec['points'] < int(self.record):
